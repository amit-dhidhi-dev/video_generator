...
//   //     console.log('Slide duration:', duration, ' controls: ', controls.slideDuration);
...
//   //       const format = selectedMimeType.includes('video/mp4') ? 'MP4' : 'WebM';
...
//   // Generate video using MediaRecorder with accurate timer-based frame drawing
//   const generateVideo = async () => {
//     if (slides.length === 0) {
//       alert('Please add some content first.');
//       return;
...
//       console.log('Total expected duration:', totalDuration, 'seconds');
...
//           // Create audio element with very low volume (almost muted but not completely)
//           audioElement = new Audio();
//           const audioUrl = URL.createObjectURL(voice);
//           audioElement.src = audioUrl;
//           audioElement.volume = 0.01; // Very low volume so user doesn't hear much
...
//           // Create gain node to boost audio for recording
//           const gainNode = audioContext.createGain();
//           gainNode.gain.value = 100; // Boost for recording (compensates for 0.01 volume)
...
//           if (audioTrack) {
//             finalStream = new MediaStream([videoTrack, audioTrack]);
//             addDebugLog('Audio track successfully added to stream!');
//           } else {
//             addDebugLog('Warning: Audio track nahi mila');
//           }
...
//         const blob = new Blob(chunks, { type: selectedMimeType });
//         addDebugLog(`Video blob: ${blob.size} bytes, expected duration: ${totalDuration}s`);
...
//       addDebugLog(`${imageElements.length} images loaded, total duration: ${totalDuration}s`);
...
//       // NEW: Use accurate timing with timestamps
//       let startTime = performance.now();
//       let currentSlide = 0;
//       let frameCount = 0;
//       const framesPerSlide = Math.round(duration * fps);

//       const drawFrame = (currentTime) => {
//         // Calculate exact frame number based on elapsed time
//         const elapsedTime = (currentTime - startTime) / 1000; // in seconds
//         const totalFramesElapsed = Math.round(elapsedTime * fps);

//         // Calculate current slide and frame
//         const slideIndex = Math.floor(totalFramesElapsed / framesPerSlide);
...
//         // Only redraw if slide changed
//         if (slideIndex !== currentSlide) {
//           currentSlide = slideIndex;
//           addDebugLog(`Slide ${currentSlide + 1} started`);
//         }
...
//       // NEW: Auto-stop after exact duration
//       setTimeout(() => {
//         addDebugLog(`Auto-stopping after ${totalDuration} seconds`);
//         mediaRecorder.stop();
//         imageElements.forEach(elem => {
//           if (elem && elem.url) URL.revokeObjectURL(elem.url);
//         });
//       }, totalDuration * 1000 + 500); // +500ms buffer
...
//   //     const fps = 30;
//   //     const slideDuration = controls.slideDuration || 3;
//   //     const totalVideoDuration = slides.length * slideDuration;

//   //     console.log('Total expected duration:', totalVideoDuration, 'seconds');
...
//   //     if (voice) {
//   //       try {
//   //         addDebugLog('Audio file detect हुई, duration check कर रहे हैं...');
...
//   //         audioContext.close();
//   //         addDebugLog(`Audio duration: ${audioDuration.toFixed(2)} seconds`);

//   //       } catch (audioError) {
//   //         addDebugLog(`Audio duration error: ${audioError.message}`);
//   //       }
//   //     }

//   //     // Decide final duration (use video or audio whichever is longer)
//   //     const finalDuration = voice ? Math.max(totalVideoDuration, audioDuration) : totalVideoDuration;
//   //     addDebugLog(`Final duration: ${finalDuration.toFixed(2)} seconds`);

//   //     // Check for best supported codecs for external player compatibility
//   //     const codecPreferences = [
//   //       // Best for compatibility (MP4 with H.264 + AAC)
//   //       'video/mp4;codecs="avc1.42E01E,mp4a.40.2"',
//   //       'video/mp4;codecs="avc1.42E01E,opus"',
//   //       'video/mp4;codecs="avc1.42E01E"',
//   //       'video/webm;codecs="vp8,opus"',
...
//   //       try {
//   //         addDebugLog('Audio merging with video...');
...
//   //         // Create final stream with both tracks
//   //         finalStream = new MediaStream([videoTrack, audioTrack]);
...
//   //         addDebugLog('Audio successfully merged without playback!');
...
//   //       mimeType: selectedMimeType,
//   //       videoBitsPerSecond: 4000000, // Higher bitrate for better quality
//   //     };

//   //     // Add audio bits per second if audio exists
//   //     if (voice) {
...
//   //         chunks.push(e.data);
//   //         addDebugLog(`Data chunk: ${(e.data.size / 1024).toFixed(2)} KB`);
//   //       }
//   //     };
...
//   //       if (chunks.length === 0) {
//   //         setLoadError('No video data received');
//   //         setIsLoading(false);
//   //         setStatus('error');
//   //         return;
//   //       }
...
//   //       // If not MP4 with proper codec, convert it
//   //       let finalBlob = blob;
//   //       let format = 'WebM';
...
//   //         resolution: '1280x720',
//   //         format: format,
//   //         codec: selectedMimeType.includes('avc1') ? 'H.264/AAC' : 'VP8/Opus',
//   //         hasAudio: !!voice
//   //       });
...
//   //       addDebugLog('Video ready! Downloading...');
//   //       downloadVideo();
//   //     };

//   //     mediaRecorder.onerror = (e) => {
//   //       addDebugLog(`MediaRecorder error: ${e.error || 'Unknown error'}`);
//   //       setLoadError('Recording error');
//   //       setIsLoading(false);
//   //       setStatus('error');
//   //       audioCleanup();
//   //     };
...
//   //     addDebugLog(`${imageElements.filter(Boolean).length}/${slides.length} images loaded`);

//   //     addDebugLog('Recording starting...');
//   //     mediaRecorder.start(1000); // Collect data every second

//   //     // Timing control for accurate duration
//   //     let startTime = performance.now();
...
//   //       // Stop if we've reached the final duration
//   //       if (elapsedTime >= finalDuration) {
//   //         addDebugLog(`Final duration reached: ${elapsedTime.toFixed(2)}s`);
//   //         mediaRecorder.stop();
...
//   //         slideIndex = Math.floor(elapsedTime / slideDuration);
//   //       } else {
//   //         // Video content finished but audio still playing
//   //         // Show last slide or black screen
//   //         slideIndex = slides.length - 1;
//   //       }

//   //       // Only redraw if slide changed or enough time passed for smooth video
//   //       if (slideIndex !== lastSlideIndex || (currentTime - lastFrameTime) >= frameInterval) {
//   //         lastSlideIndex = slideIndex;
//   //         lastFrameTime = currentTime;

//   //         if (slideIndex < slides.length) {
//   //           const imgData = imageElements[slideIndex];
//   //           if (imgData && imgData.img) {
//   //             ctx.clearRect(0, 0, canvas.width, canvas.height);
//   //             ctx.drawImage(imgData.img, 0, 0, canvas.width, canvas.height);
//   //           } else {
...
//   //       // Update progress
//   //       const progressPercent = Math.min(95, 30 + ((elapsedTime / finalDuration) * 65));
...
//   //     // Start drawing
//   //     addDebugLog('Drawing started...');
//   //     requestAnimationFrame(drawFrame);

//   //     // Auto-stop timeout as safety measure
//   //     const stopTimeout = setTimeout(() => {
//   //       if (mediaRecorder.state === 'recording') {
//   //         addDebugLog('Auto-stopping after timeout');
//   //         mediaRecorder.stop();
...
//   //         });
//   //       }
//   //     }, (finalDuration * 1000) + 2000); // 2 second buffer

//   //     // Store timeout ID to clear if manual stop happens
...
export default function VideoGenerator() {
  ...
  //   try {
  //     addDebugLog('🔄 FFmpeg loading शुरू...');
  ...
  //     addDebugLog('Core files download कर रहे हैं...');

  //     const coreURL = await toBlobURL(`${baseURL}/ffmpeg-core.js`, 'text/javascript');
  //     addDebugLog('✓ Core JS loaded');

  //     const wasmURL = await toBlobURL(`${baseURL}/ffmpeg-core.wasm`, 'application/wasm');
  //     addDebugLog('✓ WASM loaded');
  //     const classWorkerURL = await toBlobURL(`https://unpkg.com/@ffmpeg/ffmpeg@0.12.2/dist/esm/worker.js`, 'text/javascript');
  //     addDebugLog('FFmpeg initialize कर रहे हैं...');
  //     await ffmpeg.load({
  //       coreURL,
  //       wasmURL,
  //       classWorkerURL,
  ...
  //     addDebugLog('✅ FFmpeg successfully loaded और ready है!');
  //   } catch (error) {
  //     console.error('FFmpeg load error:', error);
  //     addDebugLog(`❌ FFmpeg load error: ${error}`);
  //     addDebugLog(`Error stack: ${error.stack}`);
  //     setLoadError(`FFmpeg load failed: ${error.message}. Browser compatibility issue ho sakti hai.`);
  //   }
  // };
    ...
    try {
      ...
      ffmpeg.on('progress', ({ progress, time }) => {
        if (status === 'encoding') {
          ...
          addDebugLog(
            `Encoding: ${Math.round(progress * 100)}% (${(time / 1_000_000).toFixed(1)}s)`
          );
        }
      });
      ...
      console.error('FFmpeg load error:', error);
      setLoadError(`FFmpeg load failed: ${error.message}`);
    }
  };
        ...
        <div className="text-center mb-8">
          ...
          {!ffmpegLoaded && (
            <div className="mt-4 bg-yellow-500/20 border border-yellow-500/50 rounded-lg p-4">
              <p className="text-yellow-200 flex items-center justify-center gap-2 mb-2">
                <Loader className="w-4 h-4 animate-spin" />
                FFmpeg loading... कृपया थोड़ा इंतज़ार करें (15-30 seconds)
              </p>
              <p className="text-yellow-300 text-sm text-center">
                First time load में समय लग सकता है। Debug log नीचे देखें।
              </p>
            </div>
          )}
          {ffmpegLoaded && (
            <div className="mt-4 bg-green-500/20 border border-green-500/50 rounded-lg p-3">
              <p className="text-green-200 text-center font-semibold">
                ✅ FFmpeg ready! Video generate करने के लिए तैयार है।
              </p>
            </div>
          )}
        </div>
